...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # 保存输出的日志
        with open(str(save_dir) + "/result.log", 'a+') as result_file:
            # Process predictions
...
def parse_opt():
    ...
    parser.add_argument('--conf-thres', type=float, default=0.45, help='confidence threshold | 置信度阈值，默认0.25')
    parser.add_argument('--iou-thres', type=float, default=0.45, help='NMS IoU threshold')
    parser.add_argument('--max-det', type=int, default=1000, help='maximum detections per image | 每张图像的最大检测数量')
